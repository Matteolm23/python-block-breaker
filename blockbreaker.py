...
class BALL():
    ...
    def step(self,paddle,blocks,bc,br):
        ...
        if not BALL.start:
            ...
            for i in range(len(blocks)):
                for j in range(len(blocks[i])):
                    block = blocks[i][j]
                    blockrect = g.Rect(block.x,block.y,block.width,block.height)
                    myrect = g.Rect(self.x+self.vel[0],self.y+self.vel[1],self.size+self.vel[0],self.size+self.vel[1])

                    if myrect.colliderect(blockrect) and self.killcooldown < 1:
                        cbottom = abs(myrect.bottom - blockrect.top)
                        ctop = abs(myrect.top - blockrect.bottom)
                        cright = abs(myrect.right - blockrect.left)
                        cleft = abs(myrect.left - blockrect.right)
...
class LOGIC():  
    ...
    def step(self):
        ...
        self.paddle.step()
        self.ball.step(self.paddle,self.blocks,self.bc,self.br)
